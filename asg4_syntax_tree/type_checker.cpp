...
void type_checker::write_symbol(ofstream& out, astree* node, symbol_table* sym_table,
                  const string* s) {
...
void type_checker::write_struct_field(ofstream& out, astree* node, int depth,
                        const string* name) {
...
void type_checker::write_struct(ofstream& out, astree* node, int depth) {
...
void type_checker::write_tree(ofstream& out, astree* node, int depth) {
   switch (node->symbol) {
      case TOK_STRUCT:
         if (!node->attributes[ATTR_array]) write_struct(out, node, depth);
         break;
      default:
...
void type_checker::parse_struct_child(astree* node, const string* name,
                        symbol_table* fields) {
   if (node == nullptr) return;
