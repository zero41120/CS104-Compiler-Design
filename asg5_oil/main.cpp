...
ofstream oil_file;         // use gen_oil
ofstream sym_file;         // use write_tree
...
void open_files(char* name){
   ...
   sym_file.open(output_sym, ios::out);
   oil_file.open(output_oil, ios::out);
}
...
(string_set& set, type_checker& checker, oil_language& oiler){
   ...
   /* SYM: type_cheker parse tree */
   checker.write_tree(sym_file, parser::root, 0);
   /* OIL: gen_oil parse tree */
   oiler.write_oil(oil_file, parser::root, 0);
}
...
int main (int argc, char** argv) {
   ...
   yyparse();
   checker.parse_tree(parser::root);

   // Output files
   make_output_files(set, checker, oiler);
